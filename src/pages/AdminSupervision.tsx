...
const AdminSupervision = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const loadedSupervisions = await getAdminSupervisions(unitFilter as 'RA' | 'SD' | 'SMP' | undefined);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await saveAdminSupervision(formData as Supervision);
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
      setLoading(true);
      try {
        await deleteAdminSupervision(id);
        await loadData();
      } catch (error) {
  ...
  const handleDownloadPDF = (supervision: Supervision) => {
    const teacher = teachers.find(t => t.id === supervision.teacherId);
    if (teacher) {
      generateSupervisionPDF(supervision, teacher, 'Administrasi');
    } else {
      ...
      {showForm && (
        <div className="card animate-fadeIn">
          <h2 className="text-xl font-semibold mb-4 text-blue-700">
            {isEditing ? 'Edit Supervisi Administrasi' : 'Tambah Supervisi Administrasi'}
          </h2>
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 m-4 max-w-4xl w-full">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium text-blue-700">Detail Riwayat Supervisi</h3>
              <button 
