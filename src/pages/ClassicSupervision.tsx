...
import { Eye, Filter, Pencil, Search, Trash, X, Download, School, ClipboardList, TrendingUp, BarChart3, FileText, Info } from 'lucide-react';
...
const ClassicSupervision = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await saveClassicSupervision(formData as Supervision);
      ...
      alert('Failed to save supervision data. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
      setLoading(true);
      try {
        await deleteClassicSupervision(id);
        await loadData();
      } catch (err) {
        console.error('Error deleting supervision:', err);
        alert('Failed to delete supervision. Please try again.');
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-blue-100 gap-4">
        <h1 className="text-2xl font-bold text-gray-800 flex items-center">
          <School className="mr-3 text-blue-600" size={28} />
          Supervisi Klasik
        </h1>
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white p-6 rounded-xl shadow-sm border border-blue-100">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-lg font-bold text-gray-800 flex items-center">
                    ...
                    Tren Kemajuan Supervisi Klasik (Per Tahun)
                  </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-gradient-to-br from-blue-500 to-blue-700 p-6 rounded-xl text-white shadow-md">
                  <h4 className="text-blue-100 text-sm font-medium mb-1">Total Penilaian Klasik</h4>
                ...
                <div className="bg-white p-6 rounded-xl border border-blue-100 shadow-sm">
                  <h4 className="text-gray-500 text-sm font-medium mb-1">Skor Klasik Tertinggi</h4>
                  <p className="text-3xl font-bold text-blue-600">
                    {supervisions.length > 0 ? Math.max(...supervisions.map(s => s.score)) : 0}
                  </p>
                  <div className="mt-4 flex items-center text-xs text-green-600 font-medium">
                    <TrendingUp size={14} className="mr-1" />
                    Standar keunggulan sekolah: 90+
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="bg-white p-6 rounded-xl shadow-sm border border-amber-100 h-full flex flex-col">
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="prose prose-sm text-gray-700 leading-relaxed">
                        <p className="font-medium text-gray-900 border-l-4 border-amber-400 pl-3 py-1 bg-amber-50/50 rounded-r-md mb-4">
                          Evaluasi Budaya Klasik {unitFilter ? `Unit ${unitFilter}` : 'Seluruh Unit'}
                        </p>
                        
                        <ul className="space-y-3 list-none p-0">
                          <li className="flex items-start">
                            <span className="h-5 w-5 rounded-full bg-amber-100 text-amber-600 flex-shrink-0 flex items-center justify-center text-[10px] font-bold mr-3 mt-0.5">1</span>
                            <span>Sistem mencatat <strong>{supervisions.length}</strong> penilaian kultur klasik yang telah diinputkan.</span>
                          </li>
                          <li className="flex items-start">
                            <span className="h-5 w-5 rounded-full bg-amber-100 text-amber-600 flex-shrink-0 flex items-center justify-center text-[10px] font-bold mr-3 mt-0.5">2</span>
                            <span>Rata-rata skor pada tahun <strong>{latestYear}</strong> mencapai <strong>{latestAvg.toFixed(1)}</strong>.</span>
                          </li>
                          {prevAvg !== null && (
                            <li className="flex items-start">
                              <span className="h-5 w-5 rounded-full bg-amber-100 text-amber-600 flex-shrink-0 flex items-center justify-center text-[10px] font-bold mr-3 mt-0.5">3</span>
                              <span>Menunjukkan tren <strong>{status.toLowerCase()}</strong> kualitas sebesar <strong>{Math.abs(diff).toFixed(1)}</strong> poin dari tahun {prevYear}.</span>
                            </li>
                          )}
                          <li className="flex items-start">
                            <span className="h-5 w-5 rounded-full bg-amber-100 text-amber-600 flex-shrink-0 flex items-center justify-center text-[10px] font-bold mr-3 mt-0.5">{prevAvg !== null ? '4' : '3'}</span>
                            <span>Pencapaian guru didominasi oleh <strong>Grade {stats.A > stats.B ? 'A' : 'B'}</strong>, mencerminkan pemahaman klasik yang solid.</span>
                          </li>
                        </ul>

                        <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                          <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Rekomendasi Budaya:</h4>
                          <p className="text-xs italic">
                            "Pertahankan tradisi keunggulan akademik klasik. Lakukan coaching sebaya antara guru Grade A dengan guru lainnya untuk memeratakan kualitas di seluruh jenjang."
                          </p>
                        </div>
                      </div>
                    );
                ...
                <div className="mt-6 pt-6 border-t border-gray-100 text-[10px] text-gray-400 flex justify-between items-center">
                  <span>Laporan Klasik Otomatis</span>
...
export default ClassicSupervision;
